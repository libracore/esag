...
frappe.pages.esagpos.on_page_load = function (wrapper) {
...
frappe.pages.esagpos.posClass = class PointOfSale {
    constructor(wrapper) {
    ...
    make_cart() {
        ...
        frappe.ui.form.on('Sales Invoice', 'selling_price_list', (frm) => {
            if(this.items && frm.doc.pos_profile && this.wrapper.find('.items-wrapper')[0]) {
...
class POSCart {
    ...
    get_gutschrift_aus_retoure() {
        ...
        if (return_invoice) {
            ...
            return `
                <div class="list-item">
                    <div class="list-item__content text-muted">Gutschrift aus Retoure</div>
    ...
    update_item(item) {
        ...
        if(item.qty > 0) {
            ...
            try {
                var item_tax_rates = JSON.parse(item.item_tax_rate);
                var item_tax_rate = item_tax_rates[Object.keys(item_tax_rates)[0]];
                item_rate = (item_rate / 100) * (100 + item_tax_rate);
                item_rate_txt = format_currency(item_rate, this.frm.doc.currency) + "\n(" + format_currency(item.rate, this.frm.doc.currency) + ")";
            } catch {}
            
            $item.find('.rate').text(item_rate_txt);
...
class POSItems {
    ...
    get_item_html(item) {
        ...
        const gross_price = item.gross_price ? format_currency(item.gross_price, this.currency):price_list_rate;
    ...
    get_items({start = 0, page_length = 40, search_value='', item_group=this.parent_item_group}={}) {
        ...
        return new Promise(res => {
            frappe.call({
                method: "esag.esag.page.esagpos.esagpos.get_items",
                freeze: true,
                args: {
                    start,
                    page_length,
